...
if [ -d "$SITE_DIR" ]; then
    ...
    # Remove raw .json — only .json.gz goes to Cloudflare (under 25 MiB limit)
    echo "  Removing raw .json for upload..."
    find "$SITE_DIR" -name "*.json" -type f ! -name "*.json.gz" -delete
fi
...
# ── Restore raw .json from .gz for local dev ────────────────────────────────
if [ -d "$SITE_DIR" ]; then
