...
# ── Gzip JSON for Cloudflare Pages (25 MiB per-file upload limit) ────────────
echo "Gzipping JSON files..."
SITE_DIR="$TEMPLATE_ROOT/site"
if [ -d "$SITE_DIR" ]; then
    # Gzip all .json → .json.gz alongside originals
    while IFS= read -r -d '' f; do
        gzip -fk "$f" 2>/dev/null || true
    done < <(find "$SITE_DIR" -name "*.json" -type f ! -name "*.json.gz" -print0)
    echo "  Gzipped $(find "$SITE_DIR" -name '*.json.gz' -type f | wc -l) files"

    # Remove originals so wrangler only uploads the .gz (under 25 MiB)
    echo "  Removing original .json files for upload..."
    while IFS= read -r -d '' f; do
        rm "$f"
    done < <(find "$SITE_DIR" -name "*.json" -type f ! -name "*.json.gz" -print0)

    # Trap to restore .json from .gz after deploy (success or failure)
    restore_json() {
        echo "Restoring .json from .gz for local dev..."
        while IFS= read -r -d '' f; do
            gunzip -kf "$f" 2>/dev/null || true
        done < <(find "$SITE_DIR" -name "*.json.gz" -type f -print0 2>/dev/null || true)
    }
