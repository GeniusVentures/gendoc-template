...
if [ -d "$SITE_DIR" ]; then
    ...
    # Tell wrangler to skip raw .json (keep .json.gz) so we stay under 25 MiB
    echo "*.json" > "$SITE_DIR/.gitignore"
    echo "  Wrote .gitignore — wrangler will skip *.json, upload only .json.gz"
fi
